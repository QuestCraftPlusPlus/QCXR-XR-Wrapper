...
public class UIHandler : MonoBehaviour
{
	...
	void Start()
	{
		...
		modToggle.onValueChanged.AddListener((bool v) => OnToggleClicked(v, modToggle));
		modpacksToggle.onValueChanged.AddListener((bool v) => OnToggleClicked(v, modpacksToggle));
		resourcePacksToggle.onValueChanged.AddListener((bool v) => OnToggleClicked(v, resourcePacksToggle));
		
		dropdownMain.onValueChanged.AddListener((int v) =>
		{
			selectedInstance = dropdownMain.value;
			UpdateDropdowns(false, null);
		});
		dropdownInstanceCreator.onValueChanged.AddListener((int v) =>
		{
			selectedInstance = dropdownInstanceCreator.value;
			UpdateDropdowns(false, null);
		});
		dropdownModInfo.onValueChanged.AddListener((int v) =>
		{
			selectedInstance = dropdownModInfo.value;
			UpdateDropdowns(false, null);
		});
		dropdownModSearch.onValueChanged.AddListener((int v) =>
		{
