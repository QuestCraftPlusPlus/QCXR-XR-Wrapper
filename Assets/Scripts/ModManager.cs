...
public class ModManager : MonoBehaviour
{
	...
	private void Start()
	{
		instanceDropdown.onValueChanged.AddListener(async (int v) =>
		{
			await Task.Delay(100);
			InstanceButton.currInstName = instanceLabel.text;
			if (modPage.activeSelf)
				await HasModCheck(currModSlug, currModVersions);
		});
		
		mainMenuModDropdown.onValueChanged.AddListener((int v) => SearchMods());
	...
	private async void CreateMods()
	{
		...
		// GetResults
		{
			...
			await queryDownload.SendWebRequest();
