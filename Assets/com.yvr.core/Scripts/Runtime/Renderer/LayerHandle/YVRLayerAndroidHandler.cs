...
namespace YVR.Core
{
    public class YVRLayerAndroidHandler : IYVRLayerHandle
    {
        ...
        [DllImport("yvrplugin")]
        private static extern void yvrDestroyRenderLayerAsync(int layerId, IntPtr containerPtr,
            Action<IntPtr> onLayerCreatedCallback);

        [DllImport("yvrplugin")]
        private static extern void yvrSetLayerMatrix(int layerId, EyeSide eyeSide, in Matrix4x4 layerVertices);
        ...
        [DllImport("yvrplugin")]
        private static extern int yvrGetLayerColorHandle(int layerId);

        [DllImport("yvrplugin")]
        private static extern int yvrGetTargetEyeLayerColorHandle(int layerId, EyeSide eyeSide);
        ...
        public void DestroyLayerAsync(int layerId, Action onLayerDestroyed = null)
        {
            ...
            yvrDestroyRenderLayerAsync(layerId, GCHandle.ToIntPtr(containerGCHandle), OnLayerDestroyed);
        }
        ...
        {
            RegisterCallback(CallbackType.DepthSet, onLayerDepthSetted);
        ...
        public int GetLayerColorHandle(int layerId) { return yvrGetLayerColorHandle(layerId); }

        public int GetLayerColorHandle(int layerId, EyeSide eyeSide)
        {
            return yvrGetTargetEyeLayerColorHandle(layerId, eyeSide);
        }
        ...
        {
            YVRLayerAndroidHandler handler = GCHandle.FromIntPtr(containerPtr).Target as YVRLayerAndroidHandler;
            handler?.TriggerAndUnRegisterCallback(CallbackType.Destroyed, null);
        }
        ...
        {
            yvrSetLayerMatrix(layerId, eyeSide, matrix);
        }
